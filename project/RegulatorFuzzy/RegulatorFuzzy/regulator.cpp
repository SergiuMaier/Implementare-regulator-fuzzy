...
// Initializarea tuturor matricelor si tablourilor folosite.
...
{
	functiiEroare[0] = trapez(-1, -1, -0.3, -0.15, e);		//NB
	functiiEroare[1] = trapez(-0.3, -0.15, -0.1, 0, e);		//NS
	functiiEroare[2] = triunghi(-0.1, 0, 0.1, e);			//ZE
	functiiEroare[3] = trapez(0, 0.1, 0.15, 0.35, e);		//PS
	functiiEroare[4] = trapez(0.2, 0.3, 1, 1, e);			//PB
}

//Fuzificarea derivatei erorii.
void DerivataFuzificare(float de, float functiiDerivataEroare[NUMAR_FUNCTII_APARTENENTA_DERIVATA_ERORII])
{
	functiiDerivataEroare[0] = trapez(-1, -1, -0.5, 0.2, de);		//NE
	functiiDerivataEroare[1] = triunghi(-0.5, 0, 0.5, de);		//ZE
	functiiDerivataEroare[2] = trapez(0.1, 0.3, 1, 1, de);		//PO
}

//Realizarea inferentei fuzzy.
